...
?
	...
	Function CheckZLibError(code%)
		Local msg$
		Select code
			Case Z_ERRNO        	msg$ = "Data file error"
			Case Z_STREAM_ERROR 	msg$ = "Data stream error"
			Case Z_DATA_ERROR   	msg$ = "Data subdata error"
			Case Z_MEM_ERROR    	msg$ = "Data memory error"
			Case Z_BUF_ERROR    	msg$ = "Data buffer error"
			Case Z_VERSION_ERROR 	msg$ = "Data version error"
			Default	
...
' Misspelled version kept for backward compatibility
Function SetZipStreamPasssword(zipUrl$, password$)
	Return SetZipStreamPassword(zipUrl, password)
End Function
...
Function ClearZipStreamPasssword(zipUrl$)
	Return ClearZipStreamPasssword(zipUrl)
End Function
