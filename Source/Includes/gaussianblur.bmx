...
'---------------------------------------------------------------------------------------
Function GaussianBlur:TPixmap(tex:TPixmap, radius:Int,clamptop:Int,clampbottom:Int,BlowOut:Int=255)
	...
	Method convolveAndTranspose(kernel:TKernel, in:TPixmap, out:TPixmap, width:Int, height:Int, alpha:Int)
		...
		For Local y:Int = 0 Until height
			...
			Local ioffset:Int = y*width
				...
				Next
				Local ia:Int
					If alpha = True Then ia = clamp(Int(a+0.5),TopClamp,BottomClamp,BlowOut) Else ia = $FF
				Local ir:Int =clamp( Int(r+0.5),TopClamp,BottomClamp,BlowOut)
				Local ig:Int = clamp(Int(g+0.5),TopClamp,BottomClamp,BlowOut)
				Local ib:Int = clamp(Int(b+0.5),TopClamp,BottomClamp,BlowOut)
...
Function Clamp:Int(val:Int,Top:Int,Bottom:Int,BlowOut:Int)
	If val < Bottom
		Return 0
	ElseIf val > Top
		Return BlowOut
